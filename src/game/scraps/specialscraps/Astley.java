...
public class Astley extends Item implements Purchasable, Monologuer {
    ...
    /**
     * Count that keeps track of the time in the item inventory
     */
    private int count = 0;
    ...
    public void tick(Location currentLocation, Actor actor) { // if not enough balance, then stop subscribe, then if next tick got balance le, tick directly start again??
        this.count++;
        if (this.count % TICKS_TO_PAY_SUBSCRIPTION == 0)
            System.out.println(this.pay_subscription(actor));
    }

    public String pay_subscription(Actor actor) { // When purchased the AI Device from computer terminal, is it directly subscribed
        ...
        if (actor.getBalance() >= CREDIT_TO_PAY_SUBSCRIPTION) {
            ...
            isSubscribed = false;
            this.count = 0;
            return "You don't have enough money to continue using our AI services. Goodbye.";
        }
    }
